sealed class Type (val tk: Tk, var wup: Any?, var wenv: Any?) {
    ...
    data class Union   (val tk_: Tk.Chr, val vec: Array<Type>): Type(tk_, null, null)
    data class Pointer (val tk_: Tk.Chr, var xscp1: Tk.Id?, var xscp2: Scp2?, val pln: Type): Type(tk_, null, null)
    ...
    data class Func (
        val tk_: Tk.Key,
        var xscp1s: Triple<Tk.Id?,Array<Tk.Id>?,Array<Pair<String,String>>?>,   // first=closure scope, second=input scopes
...
sealed class Expr (val tk: Tk, var wup: Any?, var wenv: Any?, var wtype: Type?) {
    ...
    data class New   (val tk_: Tk.Key, var xscp1: Tk.Id?, var xscp2: Scp2?, val arg: Expr.UCons): Expr(tk_, null, null, null)
    ...
    data class Func  (val tk_: Tk.Key, val type: Type.Func, val ups: Array<Tk.Id>, val block: Stmt.Block) : Expr(tk_, null, null, type)
    data class Call (
        val tk_: Tk,
        val f: Expr,
        val arg: Expr,
        var xscp1s: Pair<Array<Tk.Id>?,Tk.Id?>, // first=args, second=ret
...
sealed class Stmt (val tk: Tk, var wup: Any?, var wenv: Any?) {
    ...
    data class Var    (val tk_: Tk.Id, var xtype: Type?) : Stmt(tk_, null, null)
    ...
    data class Bcast  (val tk_: Tk.Key, var scp1: Tk.Id, val e: Expr): Stmt(tk_, null, null)
    data class Throw  (val tk_: Tk.Key): Stmt(tk_, null, null)
    data class Input  (val tk_: Tk.Key, var xtype: Type?, val dst: Expr?, val lib: Tk.Id, val arg: Expr): Stmt(tk_, null, null)
    data class Output (val tk_: Tk.Key, val lib: Tk.Id, val arg: Expr): Stmt(tk_, null, null)
